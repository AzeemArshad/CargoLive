...
const routes = [
    {
        ...
        children: [
            ...
            {
                path: 'cargo',
                loadChildren: () => __webpack_require__.e(/*! import() | cargo-cargo-module */ "cargo-cargo-module").then(__webpack_require__.bind(null, /*! ./cargo/cargo.module */ "./src/app/cargo/cargo.module.ts")).then(module => module.CargoModule)
            },
            {
                path: 'gl',
                loadChildren: () => __webpack_require__.e(/*! import() | general-ledger-general-ledger-module */ "general-ledger-general-ledger-module").then(__webpack_require__.bind(null, /*! ./general-ledger/general-ledger.module */ "./src/app/general-ledger/general-ledger.module.ts")).then(module => module.GeneralLedgerModule)
            },
            {
                path: 'main',
                loadChildren: () => __webpack_require__.e(/*! import() | main-main-module */ "main-main-module").then(__webpack_require__.bind(null, /*! ./main/main.module */ "./src/app/main/main.module.ts")).then(module => module.MainModule)
            }
        ]
    },
    {
        ...
        children: [
            {
                path: 'auth',
                loadChildren: () => __webpack_require__.e(/*! import() | authentication-authentication-module */ "authentication-authentication-module").then(__webpack_require__.bind(null, /*! ./authentication/authentication.module */ "./src/app/authentication/authentication.module.ts")).then(module => module.AuthenticationModule),
            },
            {
                path: 'maintenance',
                loadChildren: () => __webpack_require__.e(/*! import() | maintenance-maintenance-module */ "maintenance-maintenance-module").then(__webpack_require__.bind(null, /*! ./maintenance/maintenance.module */ "./src/app/maintenance/maintenance.module.ts")).then(module => module.MaintenanceModule)
            }
        ]
    }
];
...
/* harmony import */ var _permissions_permission_checker_service__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../permissions/permission-checker.service */ "./src/app/shared/permissions/permission-checker.service.ts");
...
/* harmony import */ var _dtos_auth_user_dto__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ../dtos/auth/user-dto */ "./src/app/shared/dtos/auth/user-dto.ts");
...
class AppComponentBase {
    constructor(injector) {
        this.message = sweetalert2__WEBPACK_IMPORTED_MODULE_3___default.a;
        this.appSession = new _dtos_auth_user_dto__WEBPACK_IMPORTED_MODULE_6__["UserDto"]();
        this.meridian = true;
        this.dt = new Date();
        this.tableDiv = true;
        ...
        this.click$ = [];
        this.typeheadlist = [];
        this.typeheadFormatter = (format) => format.name;
        this.DELIMITER = '-';
        this.permission = injector.get(_permissions_permission_checker_service__WEBPACK_IMPORTED_MODULE_1__["PermissionCheckerService"]);
        this.notify = injector.get(ngx_toastr__WEBPACK_IMPORTED_MODULE_2__["ToastrService"]);
        this.appSession = this.UserSession();
        this.calendar = injector.get(_ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_4__["NgbCalendar"]);
    ...
    formatNgbTime(time) {
        let timeString = time != null ? `${pad(time.hour)}:${pad(time.minute)}:${pad(time.second)}` : null;
        return new Date(new Date().toISOString().slice(0, 10) + " " + timeString);
    }
    dateTimeString(date, time) {
        let dateString = this.formatNgbDate(date);
        let timeString = time != null ? `${pad(time.hour)}:${pad(time.minute)}:${pad(time.second)}` : null;
    ...
    searchType(index, instance) {
        if (this.focus$.length <= index) {
            this.focus$.push(new rxjs__WEBPACK_IMPORTED_MODULE_8__["Subject"]());
            this.click$.push(new rxjs__WEBPACK_IMPORTED_MODULE_8__["Subject"]());
        }
        return (text$) => {
            const debouncedText$ = text$.pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["debounceTime"])(200), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["distinctUntilChanged"])());
            const clicksWithClosedPopup$ = this.click$[index].pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["filter"])(() => !instance.isPopupOpen()));
            const inputFocus$ = this.focus$[index];
            return Object(rxjs__WEBPACK_IMPORTED_MODULE_8__["merge"])(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_7__["map"])(term => (term === ""
                ? this.typeheadlist
                : this.typeheadlist.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10)));
        };
    }
}
AppComponentBase.ctorParameters = () => [
...
let MainTabService = class MainTabService {
    constructor() {
        this.tabs = [];
    }
    addTab(name, url) {
...
let PermissionCheckerService = class PermissionCheckerService {
    ...
    isGranted(claimType, claimValue) {
        let ret = false;
    ...
    isClaimValid(claimType, claimValue) {
        ...
        if (auth) {
            ...
            const s = auth.claims.find(c => c.claimType.toLocaleLowerCase() === claimType &&
                c.claimValue === claimValue);
            ret =
                auth.claims.find(c => c.claimType.toLocaleLowerCase() === claimType &&
...
PermissionCheckerService.ctorParameters = () => [
...
let MainContentComponent = class MainContentComponent {
    ...
    closeTab(index, event) {
        ...
        let url = this.activeTabUrl;
        let len = this.tabs.length;
        if (!this.tabs.find(tab => tab.url === url) && len >= 1) {
            url = this.tabs[len - 1].url;
        }
        else if (index === 0 && len === 2) {
            url = this.tabs[index + 1].url;
        }
        else if (len === 0 && !this.tabs.find(tab => tab.url === url)) {
...
let NavRightComponent = class NavRightComponent extends src_app_shared_common_app_component_base__WEBPACK_IMPORTED_MODULE_4__["AppComponentBase"] {
    constructor(injector, _router) {
        super(injector);
...
let NavItemComponent = class NavItemComponent {
    constructor(location, tabService) {
        this.location = location;
        this.tabService = tabService;
        this.nextConfig = _app_config__WEBPACK_IMPORTED_MODULE_3__["NextConfig"].config;
        this.themeLayout = this.nextConfig['layout'];
...
let ArrayToTreeConverterService = class ArrayToTreeConverterService {
    createTree(array, parentIdProperty, idProperty, parentIdValue, childrenProperty, fieldMappings) {
        let tree = [];
        let nodes = lodash__WEBPACK_IMPORTED_MODULE_2__["filter"](array, [parentIdProperty, parentIdValue]);
        lodash__WEBPACK_IMPORTED_MODULE_2__["forEach"](nodes, node => {
            let newNode = {
                data: node
            };
    ...
    mapFields(node, newNode, fieldMappings) {
        lodash__WEBPACK_IMPORTED_MODULE_2__["forEach"](fieldMappings, fieldMapping => {
...
let ButtonBusyDirective = class ButtonBusyDirective {
    constructor(_element) {
        this._element = _element;
    }
    set buttonBusy(isBusy) {
...
ButtonBusyDirective.ctorParameters = () => [
    { type: _angular_core__WEBPACK_IMPORTED_MODULE_1__["ElementRef"] }
];
tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_1__["Input"])(),
...
let CamelCaseToWordPipe = class CamelCaseToWordPipe {
    transform(value) {
...
let TreeDataHelperService = class TreeDataHelperService {
    findNode(data, selector) {
        let nodes = lodash__WEBPACK_IMPORTED_MODULE_2__["filter"](data, selector);
        if (nodes && nodes.length === 1) {
            return nodes[0];
        }
        let foundNode = null;
        lodash__WEBPACK_IMPORTED_MODULE_2__["forEach"](data, d => {
            if (!foundNode) {
...
/* harmony import */ var time_ago_pipe__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! time-ago-pipe */ "./node_modules/time-ago-pipe/esm2015/time-ago-pipe.js");
